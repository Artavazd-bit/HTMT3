...
simnormal <- foreach(jj = 1:nrow(simModels), .packages = c("lavaan", "semTools", "stringr", "boot"), .combine = "rbind") %:%
  ...
  foreach(sim_runs = 1:simrunnumer, .combine = "rbind") %dopar%
  {
    ...
    sim <- simFun(data = data_cfa, model = model_est, latent1 = "xi_1", latent2 = "xi_2", alpha = c(0.01, 0.05, 0.1), scale = FALSE, htmt2 = FALSE, seed = seed, bootruns = 500)
...
write.csv2(simnormal, "Normalsim.csv")
write.csv2(simnormal2, "Normalsimag.csv")
...
write.csv2(simnonnormal, "nonnormalsim.csv")
write.csv2(simnonnormal2, "nonnormalsimag.csv")
