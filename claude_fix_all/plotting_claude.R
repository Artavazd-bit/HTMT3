...
# Import all RDS files from the results folder
dfall <- read.csv2("results/HTMTsimresults_all.csv")

dfall$upperwithin <- dfall$correlation < dfall$upperbound
dfall$lowerwithin <- dfall$correlation > dfall$lowerbound

dfall$correlation <- sprintf("%.2f", dfall$correlation)
...
cmpdf$tme <- as.numeric(cmpdf$time, units = "secs")
...
normalinadmis <- ggplot(missingfilt[missingfilt$datatype == "normal" & missingfilt$n == 50 & missingfilt$method == "boot" & missingfilt$missing != 0, ], aes(x = missing)) +
  geom_histogram(aes(y=after_stat((count/sum(count)) * 100)), breaks = c(1, seq(from = 10, to = 180, by =10))) +
...
nonnormalinadmis <- ggplot(missingfilt[missingfilt$datatype == "nonnormal" & missingfilt$n == 50 & missingfilt$method == "boot" & missingfilt$missing != 0, ], aes(x = missing)) +
  geom_histogram(aes(y=after_stat((count/sum(count))* 100)), breaks = c(1, seq(from = 10, to = 180, by =10))) +
  facet_grid(cols = vars(correlation), rows  = vars(n2), labeller = label_parsed) +
  theme(legend.position = "bottom") +
  scale_y_continuous(name = "Relative frequency [%]") +
