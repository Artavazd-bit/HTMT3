...
simresults <- foreach(jj = 5:5, .packages = c("lavaan", "semTools", "stringr", "boot", "foreach"), .combine = "rbind") %:%
  ...
  foreach(sim_runs = 1:100, .combine = "rbind") %dopar%
  {
    ...
    temp <- foreach(distn = 1:1, .packages = c("lavaan", "semTools", "stringr", "boot", "foreach"), .combine = "rbind") %dopar%
      {
