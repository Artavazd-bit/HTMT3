...
bootbca <- function(data, nboot, alpha = 0.05, statisticfun, ...){
  ...
  endtimebca <- Sys.time()
  starttime_bc <- Sys.time()
  bc1 <- pnorm(q = 2 * z0 + zalpha, sd = 1, mean = 0)
  bc2 <- pnorm(q = 2 * z0 + zalpha2, sd = 1, mean = 0)
  lowerbound_bc <- unname(quantile(boot$boot, probs = bc1))
  upperbound_bc <- unname(quantile(boot$boot, probs = bc2))
  endtime_bc <- Sys.time()
  tdeltabootdca <- (endtimebca - starttimebca) + tdeltaboot
  tdeltabc <- (endtime_bc - starttime_bc) + tdeltaboot
  return(list(boot = list(se = boot$se, 
              ...
              bootbc = list(se = boot$se,
                            lowerbound = lowerbound_bc,
                            upperbound = upperbound_bc,
                            time = tdeltabc, 
                            missing = boot$missing)))
} 
################################################################################
...
################################################################################
wrapper <- function(data, model, alpha, latent1, latent2, scale = FALSE, htmt2 = FALSE, nboot)
{
  ...
  list(delta = delta, 
       ...
       bcboot = list(se = out$bootbc$se,
                     lowerbound = out$bootbc$lowerbound,
                     upperbound = out$bootbc$upperbound,
                     missing = out$bootbc$missing,
                     time = out$bootbc$time))
}
################################################################################
## Defining the models
coefs <- 1
