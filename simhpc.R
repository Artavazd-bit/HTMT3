...
sim_overview_N <- foreach(jj = 1:nrow(simModels), .packages = c("lavaan", "semTools", "stringr", "boot"), .combine = "rbind") %:%
  foreach(n = c(50, 100, 200, 400), .combine = "rbind") %:%
  foreach(sim_runs = 1:1000, .combine = "rbind") %dopar%
  {
    ...
    data_cfa <- lavaan::simulateData(model = simModels$model[jj],
                                     ...
                                     skewness = NULL,
                                     kurtosis = NULL,
                                     #skewness = c(2,  0.5,   2,  1,  1, 0.5, 2),# Numeric vector. The skewness values for the observed variables. Defaults to zero.
                                     #kurtosis = c(10,   5,   8,  3,  4,   5, 1), # Numeric vector. The kurtosis values for the observed variables. Defaults to zero.
    ...
    vc_r <- calcovcov(data = data_cfa)
    gradient_htmt <- derivhtmt(data = data_cfa, model = model_est, latent1 = "xi_1", latent2 = "xi_2", scale = FALSE, htmt2 = FALSE)
    Gradient_htmt <- as.matrix(gradient_htmt$output$gradient)
    se_htmt_1 = sqrt(t(Gradient_htmt) %*% vc_r %*% Gradient_htmt / n)
    z_value_htmt_1 = (gradient_htmt$HTMT - 1)/se_htmt_1
    end_time_delta <- Sys.time()
    
    delta_delta <- end_time_delta - start_time_delta
    alpha_vector <- c(0.9, 0.95, 0.99)
    
    #CI_HTMT_upper = gradient_htmt$HTMT + qnorm(p = alpha_vector, mean = 1, sd = se_htmt_1)
    #names(CI_HTMT_upper) <- c("0.9", "0.95", "0.99")
    
    #Bootstrapping
    set.seed(seed)
    start_time_boot <- Sys.time()
    bootstrap <- boot(data_cfa, function(data, indices){calchtmt(data = data[indices,], model = model_est, latent1 = "xi_1", latent2 = "xi_2", scale = FALSE, htmt2 = FALSE)}, R = 500)
    ...
    z_value_htmt_1_bootstrap <- (gradient_htmt$HTMT - 1) / bootstrap_htmt_1_se
...
sim_overview_N <- na.omit(sim_overview_N)

sim_overview_2_N <- sim_overview_N %>% 
...
write.csv2(sim_overview, "Normalsim.csv")
write.csv2(sim_overview_2, "Normalsimag.csv")
